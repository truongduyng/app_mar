...
export async function getSerializableProducts(): Promise<SerializableProductConfig[]> {
  ...
  return allProducts.map((product) => {
    ...
    function buildSlides(device: string, slideSet: string): SerializableSlideDef[] {
      return slides
        .filter((s) => s.device === device && s.slideSet === slideSet)
        .sort((a, b) => a.sortOrder - b.sortOrder)
        .map((s): SerializableSlideDef => {
          const copies = copy.filter((c) => c.slideSet === slideSet && c.slideKey === s.slideKey);
          const primaryCopy = copies[0];
          if (!primaryCopy) throw new Error(`No copy for ${pid}/${slideSet}/${s.slideKey}`);
    ...
    // Determine which slide sets exist for this product
    const slideSets = [...new Set(slides.map((s) => s.slideSet))];
    const localeSets = slideSets.filter((s) => s !== "default");

    const slidesByLocale: Record<string, { iphone: SerializableSlideDef[]; android?: SerializableSlideDef[] }> = {};
    for (const ls of localeSets) {
      const iphoneSlides = buildSlides("iphone", ls);
      const androidSlides = buildSlides("android", ls);
      slidesByLocale[ls] = {
        iphone: iphoneSlides,
