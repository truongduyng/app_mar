    ...
    <div style={{ padding: "24px 24px 32px", maxWidth: 1600, margin: "0 auto" }}>
      ...
      {bulkUploadOpen && (
        <BulkUploadModal
          ...
          onDone={() => { setBulkUploadOpen(false); onSlidesChanged?.(); }}
        />
      )}
...
function BulkUploadModal({ theme: T, productId, productName, productDescription, device, locale, styleKey, onClose, onDone }: {
  ...
  function setStatus(id: string, status: BulkItem["status"], error?: string) {
  ...
  async function handleRun() {
    ...
    for (const item of items) {
      ...
      // Step 1: upload + create slide
      setStatus(item.id, "uploading");
      const slideKey = `${item.label.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}-${Date.now()}`;
      const form = new FormData();
      form.append("productId",    productId);
      form.append("componentKey", styleKey);
      form.append("device",       device);
      form.append("slideKey",     slideKey);
      form.append("label",        item.label);
      form.append("file",         item.file);

      let createdSlideKey: string | null = null;
      try {
        const res  = await fetch("/api/slides/add", { method: "POST", body: form });
        const data = await res.json() as { ok?: boolean; error?: string };
        if (!res.ok || !data.ok) throw new Error(data.error ?? "Upload failed");
        createdSlideKey = slideKey;
      } catch (e) {
        setStatus(item.id, "error", e instanceof Error ? e.message : "Upload failed");
        continue;
      }
      ...
      try {
        ...
        const data = await res.json() as { ok?: boolean; headline?: string; subtitle?: string; error?: string };
        if (res.ok && data.ok && createdSlideKey) {
          // Patch the copy via /api/slide-copy
          await fetch("/api/slide-copy", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              productId,
              slideKey: createdSlideKey,
              locale,
              label: item.label,
              headline: data.headline ? [{ t: "text", v: data.headline }] : [],
              subtitle: data.subtitle ? [{ t: "text", v: data.subtitle }] : [],
            }),
          });
        }
      } catch { /* non-fatal — slide was still created */ }
  ...
  return (
    ...
    >
      <div style={{ background: "#111114", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, width: "min(680px,calc(100vw - 48px))", maxHeight: "calc(100vh - 64px)", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,0.8)" }}>
        ...
        <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, color: "#fff" }}>Bulk Upload Screenshots</div>
            <div style={{ fontSize: 12, color: "#666", marginTop: 2 }}>Upload multiple images — AI generates label, headline & subtitle for each</div>
          </div>
        ...
        <div
          ...
          style={{
            margin: "16px 16px 0",
            border: `1.5px dashed ${dragging ? T.accent : "rgba(255,255,255,0.15)"}`,
            borderRadius: 10,
            background: dragging ? `${T.accent}0a` : "rgba(255,255,255,0.02)",
            padding: "20px 16px",
            display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
            cursor: "pointer", transition: "all 0.15s",
          }}
        >
          <svg width={24} height={24} viewBox="0 0 24 24" fill="none" stroke={dragging ? T.accent : "rgba(255,255,255,0.25)"} strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>
          </svg>
          <div style={{ fontSize: 13, color: dragging ? T.accent : "rgba(255,255,255,0.35)", fontWeight: 500 }}>
            {dragging ? "Drop to add" : "Drop images here or click to select"}
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
          {items.length === 0 && (
            <div style={{ color: "#444", fontSize: 13, textAlign: "center", padding: "24px 0" }}>No images added yet</div>
          )}
          {items.map((item) => (
            <div key={item.id} style={{
              display: "flex", alignItems: "center", gap: 12,
              background: item.status === "error" ? "rgba(239,68,68,0.06)" : "rgba(255,255,255,0.03)",
              border: `1px solid ${item.status === "error" ? "rgba(239,68,68,0.2)" : item.status === "done" ? "rgba(34,197,94,0.2)" : "rgba(255,255,255,0.07)"}`,
              borderRadius: 8, padding: "10px 12px",
            }}>
              {/* Thumbnail */}
              <img src={item.preview} alt="" style={{ width: 36, height: 60, objectFit: "cover", objectPosition: "top", borderRadius: 4, flexShrink: 0 }} />
              ...
              <input
                ...
                style={{
                  ...
                  borderRadius: 6, padding: "6px 10px", fontSize: 12, color: "#fff",
                  fontFamily: "inherit", outline: "none",
                  opacity: (running || item.status === "done") ? 0.6 : 1,
                }}
              />
              ...
              <div style={{ flexShrink: 0, width: 100, textAlign: "right" }}>
                {item.status === "pending" && (
                  <span style={{ fontSize: 11, color: "#555" }}>Ready</span>
                )}
                {item.status === "uploading" && (
                  <span style={{ fontSize: 11, color: T.accent }}>Uploading…</span>
                )}
                {item.status === "generating" && (
                  <span style={{ fontSize: 11, color: T.accent }}>
                    <svg width={10} height={10} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} style={{ animation: "spin 1s linear infinite", verticalAlign: "middle", marginRight: 4 }}>
                      <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/>
                    </svg>
                    AI copy…
                  </span>
                ...
                {item.status === "error" && (
                  <span style={{ fontSize: 11, color: "#FCA5A5" }} title={item.error}>✗ Error</span>
                )}
              </div>
              ...
              {!running && item.status !== "done" && (
                <button
                  ...
                  style={{ background: "none", border: "none", color: "#555", cursor: "pointer", fontSize: 16, lineHeight: 1, padding: "0 2px", flexShrink: 0 }}
                  title="Remove"
                >×</button>
              )}
            </div>
        ...
        {/* Footer */}
        <div style={{ padding: "12px 16px", borderTop: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div style={{ fontSize: 12, color: "#555" }}>
            {items.length > 0 ? `${items.length} image${items.length > 1 ? "s" : ""} · AI will generate copy for each` : ""}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={onClose}
            ...
            <button
              ...
              style={{
                background: (running || pendingCount === 0) ? "rgba(255,255,255,0.08)" : `linear-gradient(135deg, ${T.accent}, ${T.accent}dd)`,
                color: (running || pendingCount === 0) ? "#555" : "#fff",
                border: "none", borderRadius: 8, padding: "8px 20px",
                fontSize: 13, fontWeight: 700, cursor: (running || pendingCount === 0) ? "not-allowed" : "pointer",
                boxShadow: (running || pendingCount === 0) ? "none" : `0 4px 16px ${T.accentGlow}`,
                transition: "all 0.15s", minWidth: 140,
              }}
            >
              {running
                ? `Processing ${items.filter((i) => i.status === "done").length}/${items.length}…`
                : `Upload ${pendingCount} slide${pendingCount !== 1 ? "s" : ""} & Generate`}
            </button>
          </div>
        </div>
      </div>
